...
import { removeBackgroundCanvas, removeBackgroundByColorsCanvas, paintBackground, hexToRgb } from "./lib/background";
import {
  readAsBitmap,
  download,
  fmtSize,
...
export default function App() {
  ...
  const loadFile = useCallback(async (file: File) => {
    ...
    setExportInfo("");
    if (!acceptMime(file)) {
      setError("目前支持 PNG、JPG、WebP、GIF、BMP、AVIF、TIFF 图片。");
      setLoading(false);
      return;
    }
    try {
  ...
  const bgMask = useMemo(() => {
    ...
    if (bgStrategy === "model") return modelMask;
    if (bgStrategy === "color" && (!sampleColors.length || sampleColors.some((color) => !color))) return null;
    const cached = bgMaskCacheRef.current;
    const same = cached && cached.source === rotated && cached.strategy === bgStrategy && cached.fuzz === fuzz && JSON.stringify(cached.colors) === JSON.stringify(sampleColors);
    if (same) return cached.mask;
    if (bgMode === "original") return null;
    const colors = sampleColors.map(hexToRgb);
    const mask = bgStrategy === "edge"
      ? removeBackgroundCanvas(rotated, fuzz)
      : removeBackgroundByColorsCanvas(rotated, colors, fuzz);
    bgMaskCacheRef.current = { source: rotated, strategy: bgStrategy, fuzz, colors: [...sampleColors], mask };
    return mask;
  }, [rotated, bgMode, bgStrategy, fuzz, sampleColors, modelMask]);
  ...
  const onBoxMove = (e: React.PointerEvent) => {
    ...
    const { startX, startY, crop: c0 } = dragState.current;
    const scale = disp.scale || 1;
    ...
    const hasW = HANDLE.includes("w") as boolean; // "nw","sw","w"
    const hasE = HANDLE.includes("e") as boolean;
    const hasN = HANDLE.includes("n") as boolean;
    const hasS = HANDLE.includes("s") as boolean;

    if (HANDLE === "move") {
      let nx = c0.x + dx;
      let ny = c0.y + dy;
      nx = Math.max(0, Math.min(nx, W - c0.w));
      ny = Math.max(0, Math.min(ny, H - c0.h));
      setCrop({ x: nx, y: ny, w: c0.w, h: c0.h });
      return;
    }
    ...
    if ((horizontal && Math.abs(signedDx) < Math.abs(signedDy) * r) || (!horizontal && Math.abs(signedDy) < Math.abs(signedDx) / r)) {
      nw = widthFromY;
    }
    ...
    const nh = nw / r;
    const nx = hasW ? anchorX - nw : anchorX;
    const ny = hasN ? anchorY - nh : anchorY;
        ...
        <main className="wrap">
          ...
          >
            ...
            <input
              ref={fileRef}
              type="file"
              accept=".png,.jpg,.jpeg,.webp,.gif,.bmp,.avif,.tif,.tiff,.heic,.heif,image/*"
              hidden
              onChange={(e) => {
          ...
          <section className="stage-wrap">
            <div className={`stage${src.hasTransparency || bgMode === "remove" ? " stage-checker" : ""}`} ref={stageRef} onPointerMove={stageHover} onPointerLeave={() => { if (stageRef.current) stageRef.current.style.cursor = ""; }}>
              <div className={`stage-inner${samplingIndex !== null ? " sampling-active" : ""}`} ref={innerRef} onClick={onStageClick} style={{ width: disp.w, height: disp.h }}>
                ...
                >
                  {/* 边带:整条边可按住调整,悬停光标即切换 */}
                  <span className="crop-edge crop-edge-n" onPointerDown={onBoxDown("n")} onPointerMove={onBoxMove} onPointerUp={onBoxUp} />
                  <span className="crop-edge crop-edge-s" onPointerDown={onBoxDown("s")} onPointerMove={onBoxMove} onPointerUp={onBoxUp} />
                  <span className="crop-edge crop-edge-e" onPointerDown={onBoxDown("e")} onPointerMove={onBoxMove} onPointerUp={onBoxUp} />
                  <span className="crop-edge crop-edge-w" onPointerDown={onBoxDown("w")} onPointerMove={onBoxMove} onPointerUp={onBoxUp} />
                  {/* 四角手柄 */}
